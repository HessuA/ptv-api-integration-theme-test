 ...
 * @Last Modified time: 2023-12-03 01:40:01
 */
...
function calculateBurgerMenuPosition() {
  ...
  if (viewportWidth <= widthMaxMobile) {
    ...
    // If there is top-nav. calculate top and height of menu-items-wrapper
    if (document.querySelector('.top-nav')) {
      ...
      document.getElementById('menu-items-wrapper').style.height = `calc(100vh - ${siteHeaderHeight + topNavHeight}px)`;
      document.getElementById('menu-items-wrapper').style.top = `${siteHeaderHeight + topNavHeight}px`;
    }
