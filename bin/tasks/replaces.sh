...
for i in `grep -rl ptv-api-test * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;ptv-api-test;${THEME_NAME};" $i $i; done
for i in `grep -rl ptv-api-test * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;ptv-api-test;${THEME_NAME};" $i $i; done
for i in `grep -rl air * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;ptv-api-test;${THEME_NAME};" $i $i; done
for i in `grep -rl air * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;Ptv_api_test_;${THEME_NAME}_;" $i $i; done
for i in `grep -rl air * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;Ptv_api_test_;${THEME_NAME}_;" $i $i; done
