#!/bin/bash
# WordPress theme starting bash script for ptv-api-test (Laragon version)
...
# Get latest ptv-api-test version with updates and copy it over to your project
...
# Create latest ptv-api-test development packages for project root level (gulp paths etc.)
source ${SCRIPTS_LOCATION}/tasks/project.sh

# Clean up leftover development files from ptv-api-test
source ${SCRIPTS_LOCATION}/tasks/cleanups.sh

# Replace ptv-api-test with your theme name and other seds (WSL version)
