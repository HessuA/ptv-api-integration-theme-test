...
} // end Ptv_api_test_move_jquery_into_footer
...
function enqueue_theme_scripts() {
  ...
  wp_localize_script( 'scripts', 'Ptv_api_test_screenReaderText', [
  ...
  wp_localize_script( 'scripts', 'Ptv_api_test_externalLinkDomains', THEME_SETTINGS['external_link_domains_exclude'] );
} // end Ptv_api_test_scripts
...
function enqueue_polyfills() {
  ...
  if ( file_exists( get_theme_file_path( get_asset_file( 'legacy.js' ) ) ) ) {
    wp_register_script( 'Ptv_api_test_legacy', '', [], filemtime( get_theme_file_path( get_asset_file( 'legacy.js' ) ) ), false );
    wp_enqueue_script( 'Ptv_api_test_legacy' );
    wp_add_inline_script( 'Ptv_api_test_legacy', $script, true );
  }
